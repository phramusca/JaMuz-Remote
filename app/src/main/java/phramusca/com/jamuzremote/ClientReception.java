    ...
    public void run() {
        try {
            ...
            if (ex.getCause() instanceof ErrnoException) {
                ...
                //TODO, maybe: sync and merge: Manage errors like ENOENT (No such file or directory)") : SyncStatus{status=CONNECTED, nbRetries=0}
                // => Less chance to happen since "SENDING_FILE" is not more received (replaced by API)
